...
class Render_3DMM(nn.Module):
    ...
    def __init__(self, focal=1015, img_h=500, img_w=500, batch_size=1, device=None):
    ...
    def get_render(self, batch_size=1):
        # R, T = look_at_view_transform(10, 0, 0, at=((0, 0, 0),)) # the front of the cow is facing the -z direction
        ...
        # TODO: FIXME: 9.3657
        # pytorch3d: left-hand, <-x
        # R, T = look_at_view_transform(self.focal+9.3657, 0, 0, at=((0, 0, -self.focal),)) # the front of the cow is facing the -z direction
        R, T = look_at_view_transform(self.focal, 0, 0, at=((0, 0, -self.focal),))
        cameras = FoVPerspectiveCameras(device=self.device, R=R, T=T, znear=0.01, zfar=20,
                                        fov=2*np.arctan(self.img_w//2/self.focal)*180./np.pi)
    ...
    def get_render_ori(self, batch_size=1):
        half_s = self.img_w * 0.5
        R, T = look_at_view_transform(10, 0, 0) # the front of the cow is facing the -z direction
