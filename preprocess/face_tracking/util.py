...
def proj_pts(rott_geo, focal_length, cxy):
    ...
    fxX = focal_length*X
    fyY = focal_length*Y
    proj_x = -fxX/Z + cx # pixel coordinate system
    proj_y = fyY/Z + cy # pixel coordinate system
    return torch.cat((proj_x[:, :, None], proj_y[:, :, None], Z[:, :, None]), 2)

def forward_rott(geometry, euler_angle, trans, return_pose=False, change_coord_hand=False):
    rot = euler2rot(euler_angle, change_coord_hand=change_coord_hand)
